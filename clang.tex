...
int main(void) {
    ...
    fgets(pdata.family_name, sizeof(pdata.family_name), stdin);
    printf("Input given name: ");
    fgets(pdata.given_name, sizeof(pdata.given_name), stdin);
    printf("Input age: ");
...
\end{verbatim}
\end{quote}
のように \texttt{mat\_elem} マクロを定義することである\footnote{一般的にCプロセッサのマクロの多用は勧められていないが、背に腹は代えられない。}。これにより、プログラム中で行列の\((i,j)\)成分を \texttt{mat\_elem(A, i, j)} と書けるようになる。
