...
なお、\underline{\texttt{cd}}とだけ打てば、いつでも自分のホームディレクトリに戻ってくることができる。ホームディレクトリは \(\tilde{\ }\) とも表されるので \underline{\texttt{cd \(\tilde{\ }\)}}としても同じである。他にも特殊な記法として``前のカレントディレクトリ''を表す\texttt{ - }が使用できる。
